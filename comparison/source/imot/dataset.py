...
class Dataset:
    ...
    def __init__(
        ...
        for session_path in path_data.iterdir():
            ...
            while idx + window_size < len_session:
                ...
                self.x_a_a.append(acc)
                self.x_a_g.append(gyro)
                self.y.append(velocity[idx + window_size - 1])

                idx += window_step

        self.x_a_a = np.stack(self.x_a_a).astype(np.float32)
        self.x_a_g = np.stack(self.x_a_g).astype(np.float32)
    ...
    def __getitem__(
        ...
        return self.x_a_a[idx], self.x_a_g[idx], self.y[idx]