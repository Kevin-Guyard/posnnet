...
class ConvolutionalLongShortTermMemoryTransformerFrequencyAwareWithBypassHpSpace:
    ...
    def __init__(
        ...
        kernel_size_conv_1_subspace: IntegerSpace=IntegerSpace(name="kernel_size_conv_1", low=3, high=9, step=2),
        kernel_size_conv_2_subspace: IntegerSpace=IntegerSpace(name="kernel_size_conv_2", low=3, high=9, step=2),
        ...
        """
        Initiate the hyperparameters space for the CLSTMTFAWB model.
        For a more complete hyperparameters descriptions, check the class docstring in posnnet.models.convolutional_long_short_term_memory_transformer_frequency_aware_with_bypass.
        ...
        Args:
            ...
            - kernel_size_conv_1_subspace (posnnet.hp_spaces.subspaces.IntegerSpace): The subspace for the size of the kernel of the first convolution of the CNN.
                                                                                      Default: IntegerSpace(name="kernel_size_conv_1", low=3, high=9, step=2) = [low, low + 2, low + 4, ..., high].
            - kernel_size_conv_2_subspace (posnnet.hp_spaces.subspaces.IntegerSpace): The subspace for the size of the kernel of the second convolution of the CNN.
                                                                                      Default: IntegerSpace(name="kernel_size_conv_2", low=3, high=9, step=2) = [low, low + 2, low + 4, ..., high].
        ...
        self.kernel_size_conv_1_subspace = kernel_size_conv_1_subspace
        self.kernel_size_conv_2_subspace = kernel_size_conv_2_subspace
    ...
    def sample_model_hp(
        ...
        **kwargs: Any
        ...
        Args:
            ...
            - min_len_seq (int): The minimal length of a sequence (min length = min GPS outage length + min side length).

        Returns:
            - model_params(Dict[str, Any]): The input dictionnary with the following new keys: 'd_model', 'n_head',
                                            'hidden_size_ff', 'kernel_size_conv_1', 'kernel_size_conv_2', 'n_encoder_layers',
                                            'n_decoder_layers' and 'p_dropout'.
        """
        ...
        # Sample the size of the kernels for the CNN.
        kernel_size_conv_1 = self.kernel_size_conv_1_subspace.sample_value(trial=trial)
        kernel_size_conv_2 = self.kernel_size_conv_2_subspace.sample_value(trial=trial)

        # Sample the number of encoders and decoders in the model.
        # Because each encoder layer will divide the length of the sequence by 2, the maximum number of encoder layers is constrained by the minimal sequence length.
        n_encoder_layers = self.n_encoder_layers_subspace.sample_value(trial=trial, constraint_high=int(np.log2(min_len_seq)))
