...
class ConvolutionalLongShortTermMemoryTransformerWithBypass(torch.nn.Module):
    ...
    def __init__(
        ...
        """
        Initialize a Convolutional Long Short Term Memory Transformer (CLSTMT) with a bypass path which follow the structure:
        Encoder = -> Linear projection in encoder -> Position encoding -> Encoder layer 1 -> ... -> Encoder layer N -> Decoder memory
        ...
        super(ConvolutionalLongShortTermMemoryTransformerWithBypass, self).__init__()

        # Linear projection from input space to model space
        self.linear_projection_in_encoder = torch.nn.Linear(in_features=input_size + 1, out_features=d_model) # +1 because of the mask in addition of the sensor features
        ...
        self.lstm_position_encoding_encoder = torch.nn.LSTM(
            input_size=d_model,
            hidden_size=d_model,
            num_layers=1,
            bias=True,
    ...
    def __init_weights(
        ...
        torch.nn.init.xavier_uniform_(self.linear_projection_in_encoder.weight)
        torch.nn.init.constant_(self.linear_projection_in_encoder.bias, 0.0)
        ...
        for module in self.lstm_position_encoding_encoder.modules():
    ...
    def forward(
        ...
        memory = x # size = (batch_size, len_seq, input_size + 1)
        memory = self.linear_projection_in_encoder(memory) # size = (batch_size, len_seq, d_model)
        memory = memory + self.lstm_position_encoding_encoder(memory)[0] # size = (batch_size, len_seq, d_model)
        memory = self.encoder(memory) # size = (batch_size, len_seq, d_model)
