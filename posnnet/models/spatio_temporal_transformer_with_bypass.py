...
class SpatioTemporalTransformerWithBypass(torch.nn.Module):
    ...
    def __init__(
        ...
        """
        Initialize a Spatio Temporal Transformer (STT) with a bypass path which follows the structure:
        Encoder = -> Spatial embedding -> Spatial encoder layer 1 -> ... -> Spatial encoder layer N -> Temporal decoder memory
        ...
        super(SpatioTemporalTransformerWithBypass, self).__init__()
    ...
    def forward(
        ...
        x_spatial = x_temporal.swapaxes(1, 2) # size = (batch_size, input_size + 1, len_seq)
        x_spatial = self.spatial_embedding(x_spatial) # size = (batch_size, d_model, len_seq)
        x_spatial = self.spatial_encoder(x_spatial) # size = (batch_size, d_model, len_seq)
        x_spatial = x_spatial.swapaxes(1, 2) # size = (batch_size, len_seq, d_model)
